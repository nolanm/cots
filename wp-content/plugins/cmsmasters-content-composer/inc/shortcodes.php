...
public function cmsmasters_contact_form($atts, $content = null) {
	...
	if ($form_plugin == 'cf7' && $form_cf7 != '') {
		...
		$out .= do_shortcode('[cmsmasters_contact_form_sc formname="' . esc_attr($form_cfb) . '" email="' . esc_attr($email_cfb) . '"]');
	}
...
public function cmsmasters_quote($atts, $content = null) {
	...
	'">' . "\n" . 
		...
		cmsmasters_composer_ob_load_template('theme-framework/theme-style' . CMSMASTERS_CONTENT_COMPOSER_THEME_STYLE . '/postType/quote/quote-' . $this->quotes_atts['quote_mode'] . '.php', $this->quotes_atts) . 
...
public function cmsmasters_social($atts, $content = null) {
	...
	if ($facebook == 'true' || $twitter == 'true' || $google == 'true' || $pinterest == 'true') {
		...
		if ($google == 'true') {
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="https://plus.google.com/share?url=' . $page_link . '" class="button cmsmasters-icon-google">' . esc_html__('Google+', 'cmsmasters-content-composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
		...
		if ($pinterest == 'true') {
			...
			$out .= '<div class="share_wrap">' . "\n" . 
				'<a href="https://pinterest.com/pin/create/button/?url=' . $page_link . '&media=' . $pinterest_img . '&description=' . $social_title . '" class="button cmsmasters-icon-pinterest">' . esc_html__('Pinterest', 'cmsmasters-content-composer') . '</a>' . "\n" . 
			'</div>' . "\n";
		}
			...
			">";
				...
				if ($post_type == 'project') {
					while ($query->have_posts()) : $query->the_post();
						...
						$out .= '<div class="cmsmasters_owl_slider_item">' . 
							cmsmasters_composer_ob_load_template('theme-framework/theme-style' . CMSMASTERS_CONTENT_COMPOSER_THEME_STYLE . '/postType/posts-slider/slider-project.php', $this->posts_slider_atts) . 
						'</div>';
	...
	if ($query->have_posts()) : 
		while ($query->have_posts()) : $query->the_post();
			if ($layout == 'columns') {
				...
				$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style' . CMSMASTERS_CONTENT_COMPOSER_THEME_STYLE . '/postType/blog/post-' . $layout . '.php', $this->blog_atts);
			} else {
...
public function cmsmasters_profiles($atts, $content = null) { 
	...
	if ($query->have_posts()) :
        ...
        while ($query->have_posts()) : $query->the_post();
			...
			if ($layout == 'vertical') {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style' . CMSMASTERS_CONTENT_COMPOSER_THEME_STYLE . '/postType/profile/profile-vertical.php', $this->profiles_atts);
			} else {
				$out .= cmsmasters_composer_ob_load_template('theme-framework/theme-style' . CMSMASTERS_CONTENT_COMPOSER_THEME_STYLE . '/postType/profile/profile-horizontal.php', $this->profiles_atts);
			}
