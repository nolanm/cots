...
switch ( $map_style ) {
    ...
    $query = new WP_Query('post_type=tribe_events&post_status=publish&ignore_sticky_posts=true&posts_per_page=');
    $address =  tribe_get_address();
    $latlong = get_latlong_geocode($address);
?>
<div class="consult-wrap-map finazi-wrap-map-<?php echo esc_attr($contact_map); ?>" style="height: <?php echo esc_attr($map_height); ?>px;">
    <?php if($contact_map!='none'):/*contact map*/ ?>
        <div class="consult-contact-map-box consult-contact-map-box-<?php echo esc_attr($contact_map); ?>">
...
$source = array('<br />');
$replace = array('');
$title = str_replace($source, $replace, $map_custom);

$map_style_snippet = json_encode($map_style_snippet);

if ( $el_class ) {
    echo 'class="finazi-gmaps ' . esc_attr( $el_class ). '"';
} else {
    echo 'id="map-canvas-event" class="finazi-gmaps"';
}
?>
    data-address="<?php echo esc_attr( $latlong ); ?>"
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBJVNvnez6TWzPAZ0BAJOSCmzf9GcmyTFw"></script>
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        var gmMapDiv = $("<?php if ( $el_class ) {echo '.' . trim($el_class);} else {echo '#map-canvas-event';} ?>");
        (function ($) {
            ...
            if (gmMapDiv.length) {
                ...
                gmMapDiv.gmap3({
                    ...
                    marker: {
                        ...
                        options: {
                            <?php if ($marker_icon == '') { ?>
                                icon: "<?php echo get_template_directory_uri(); ?>/images/icon-map.png",
                            <?php } else { ?>
                        ...
                        events: {
                            click: function (marker, event) {
                                ...
                                if (infowindow) {
                                    ...
                                    infowindow.setContent("<?php echo esc_js( $maker_info ); ?>");
                                }
                                else {
                                    $(this).gmap3({
                                        infowindow: {
                                            anchor: marker,
                                            options: {content: "<?php echo esc_js( $maker_info ); ?>"}
                                        }
                                    });
                                }
                            }
                        ...
                        options: {
                            ...
                            <?php if ($map_style != 'default') { ?>
                                <?php if ($map_style == 'custom') { ?>
                                    <?php if ($title != '') { ?>
                                        styles: <?php echo wp_kses_post($title); ?>,
                                    <?php } ?>
                                <?php } else { ?>
