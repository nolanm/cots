...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="cmsmasters_donation_cont_wrap">
		<?php 
		if ($image) {
			salvation_thumb_rollover(get_the_ID(), 'cmsmasters-project-thumb', false, $link, false, false, false, false, false, false, true, false, false, 'cmsmasters_theme_icon_person');
		}
		?>
		<div class="cmsmasters_donation_cont">
			<?php 
			$campaign ? salvation_donations_donation_campaign(get_the_ID(), 'page') : '';
			
			salvation_donations_donation_heading(get_the_ID(), 'h3', $link);
			?>
		</div>
		<?php 
		if ($amount) { ?>
		<footer class="cmsmasters_donation_footer entry-meta">
			<?php salvation_donations_donation_amount_currency(get_the_ID(), 'page'); ?>
		</footer>
		<?php } 
		?>
	</div>
