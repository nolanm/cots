...
function salvation_prev_next_posts($order_cat = false, $tax_name = 'category') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'salvation') . '<span class="post_nav_type"> ' . $cmsmasters_post_type . ' </span>' . esc_html__('Link', 'salvation') . '</span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'salvation') . '<span class="post_nav_type"> ' . $cmsmasters_post_type . ' </span>' . esc_html__('Link', 'salvation') . '</span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>', '%title', $order_cat, '', $tax_name);
...
function salvation_sharing_box($title_box = false, $tag = 'h3') {
...
function salvation_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'salvation') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function salvation_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
...
function salvation_author_avatar($template_type = 'page') {
...
function salvation_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'salvation') . '</' . esc_html($tag) . '>' . "\n" .
