...
if (
	$cmsmasters_option['salvation' . '_portfolio_project_like'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_date'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_cat'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_comment'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_author'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_tag'] || 
	$cmsmasters_option['salvation' . '_portfolio_project_link'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_post_format == 'gallery') {
		...
		salvation_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-project-full-thumb');
		...
		if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
			...
			salvation_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
		}
	}
	...
	if ($cmsmasters_project_title == 'true') {
		echo '<header class="cmsmasters_project_header entry-header">';
			salvation_project_title_nolink(get_the_ID(), 'h2');
		echo '</header>';
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_post_format == 'video') {
			...
			salvation_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
		}
		...
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				...
				echo '<div class="project_details entry-meta">';
					...
					salvation_get_project_likes('post');
					
					salvation_get_project_comments('post');
					
					salvation_get_project_author('post');
					
					salvation_get_project_date('post');
					
					salvation_get_project_category(get_the_ID(), 'pj-categs', 'post');
					
					salvation_get_project_tags(get_the_ID(), 'pj-tags');
					
					salvation_get_project_features('details', $cmsmasters_project_features, false, 'h5', true);
					
					salvation_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
					
				echo '</div>';
			}
			
			
			salvation_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h5', true);
			
			salvation_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h5', true);
			
			salvation_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h5', true);
...
if ($cmsmasters_project_sharing_box == 'true') {
	salvation_sharing_box(esc_html__('Like this project?', 'salvation'));
}


if ($cmsmasters_option['salvation' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['salvation' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['salvation' . '_portfolio_project_nav_order_cat'] : false);
	salvation_prev_next_posts($order_cat, 'pj-categs');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	salvation_related( 
		'h3', 
		esc_html__('More projects', 'salvation'), 
		esc_html__('No projects found', 'salvation'), 
		$cmsmasters_project_more_posts, 
		$tgsarray, 
		$cmsmasters_option['salvation' . '_portfolio_more_projects_count'], 
		$cmsmasters_option['salvation' . '_portfolio_more_projects_pause'], 
		'project', 
		'pj-tags' 
	);
}
