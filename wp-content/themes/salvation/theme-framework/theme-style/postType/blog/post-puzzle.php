	...
	<div class="cmsmasters_post_cont">
		...
		if (!post_password_required()) {
			if ($cmsmasters_post_format == 'image') {
				...
				salvation_post_type_image(get_the_ID(), $cmsmasters_post_image_link, 'cmsmasters-project-thumb');
			} elseif ($cmsmasters_post_format == 'gallery') {
				$cmsmasters_post_images = explode(',', str_replace(' ', '', str_replace('img_', '', get_post_meta(get_the_ID(), 'cmsmasters_post_images', true))));
				
				salvation_post_type_slider(get_the_ID(), $cmsmasters_post_images, 'cmsmasters-project-thumb');
			} elseif (has_post_thumbnail()) {
				salvation_thumb(get_the_ID(), 'cmsmasters-project-thumb', true, false, true, false, true, true, false);
			} else {
				salvation_post_format_icon_placeholder(get_the_ID(), 'image');
			}
		} else {
			salvation_post_format_icon_placeholder(get_the_ID(), 'image');
		}
		...
		echo '<div class="puzzle_post_content_wrapper">' . 
			...
			$date ? salvation_get_post_date('page', 'puzzle') : '';
			
			salvation_post_heading(get_the_ID(), 'h5');
