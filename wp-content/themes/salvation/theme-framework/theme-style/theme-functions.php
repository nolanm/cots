...
function salvation_register_theme_js_scripts() {
	...
	wp_localize_script('salvation-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'salvation_register_theme_js_scripts');


/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'salvation'), 
    'footer' => 	esc_html__('Footer Navigation', 'salvation'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'salvation') 
));
...
function salvation_fallback_menu($args) {
	$cmsmasters_option = salvation_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['salvation' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function salvation_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'salvation_single_project_layout');



/* Search Form */
function salvation_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('Search...', 'salvation') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'salvation_search_form');

/* Responsive navigation button add  */
function salvation_resp_mid_navigation() {
...
add_action('cmsmasters_after_logo', 'salvation_resp_mid_navigation', 5);